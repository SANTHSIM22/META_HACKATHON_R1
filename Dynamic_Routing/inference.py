...
SYSTEM_PROMPT = textwrap.dedent("""
    ...
    You will receive a JSON observation containing:
      ...
      - trucks         : list of trucks with id, current_location, route_order, assigned_packages
    ...
    YOUR JOB:
      Return a JSON object with route updates for all trucks.
    ...
    RESPONSE FORMAT:
    {
      "route_updates": [
        {
          "truck_id": "TRK_XXX",
          "new_route_order": ["Node_A_B", "Node_C_D", "Node_E_F"]
        }
      ]
    }
    ...
    CRITICAL RULES:
      ...
      2. A route is a sequence of NODES. Check if consecutive nodes form a blocked edge.
         Example: If ["Node_A", "Node_B"] is blocked, route [..., "Node_A", "Node_B", ...] is INVALID.
      3. Each truck must visit ALL destinations of its assigned packages.
      4. Deliver packages before their deadlines.
      5. Minimize total travel time (fuel cost) while meeting deadlines.
      6. Use EXACT truck IDs and node names from the observation.
      7. If a node has traffic_delays, account for extra travel time.
...
def parse_llm_response(text: str) -> Optional[DynamicRouteAction]:
    ...
    try:
        ...
        return DynamicRouteAction(route_updates=updates)
